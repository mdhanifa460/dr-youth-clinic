  ...
  return (
    <div className="bg-white rounded-3xl p-5 md:p-7 shadow-[0_8px_28px_rgba(11,37,96,0.07)] flex flex-col gap-4 h-full border border-gray-100 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_38px_rgba(11,37,96,0.11)]">
      ...
      {review.source === 'video' && review.videoUrl && (
        <a href={review.videoUrl} target="_blank" rel="noopener noreferrer"
          ...
          {review.videoThumbnail
            // width/height attributes here are purely for the browser's
            // aspect-ratio reservation (CSS width/height:100% still
            // controls the actual rendered size, unchanged) — without
            // them the browser allocates a 0x0 box until the image loads,
            // then the sudden expansion shifts surrounding content (CLS).
            // 640x360 matches the parent's own aspect-video (16:9) ratio.
            ? <img src={review.videoThumbnail} alt="video" width={640} height={360} className="w-full h-full object-cover" loading="lazy" decoding="async" />
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-gray-50">
        {review.authorAvatar
          // Same reasoning as the video thumbnail above — w-9/h-9 (36px)
          // is a fixed Tailwind size, but that alone doesn't stop the
          // browser reserving 0x0 before load; explicit attributes do.
          ? <img src={review.authorAvatar} alt={review.authorName} width={36} height={36} className="w-9 h-9 rounded-full object-cover shrink-0" loading="lazy" decoding="async" />
