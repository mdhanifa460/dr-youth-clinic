...
export const formSchema = z.object({
  ...
  phone: z.string().refine(isValidIndianMobile, INVALID_MOBILE_MESSAGE),
...
export const bookingSchema = z.object({
  ...
  // Real 10-digit Indian mobile validation, not just "is a plausible
  // length" — this is the one backend enforcement point every booking
  // entry point funnels through (the 4-step /book form, the homepage
  // quick form, the AI chat widget's booking form), so fixing it here
  // fixes it everywhere at once.
  phone: z.string().trim().refine(isValidIndianMobile, INVALID_MOBILE_MESSAGE),
