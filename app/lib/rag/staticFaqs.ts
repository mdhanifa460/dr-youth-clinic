// Moved verbatim out of app/(public)/faqs/page.tsx — these are fallback/seed
// FAQ content, merged there with any CMS-authored FAQs (HomepageSection
// sectionKey: 'faq'). Relocated here (rather than left inline in the page)
// so the RAG knowledge-base sync (triggered from the admin homepage PUT
// route, which cannot import a page module) can also reach this data.
export const STATIC_FAQS: { category: string; icon: string; items: { question: string; answer: string }[] }[] = [
  {
    category: 'General',
    icon: '🏥',
    items: [
...
export function flattenStaticFaqs(): { question: string; answer: string; category: string }[] {
  return STATIC_FAQS.flatMap((c) => c.items.map((i) => ({ ...i, category: c.category })));
}
